...
<jsp:useBean id="listChefSkillsType_ByCompositeQuery" scope="request"
	type="java.util.List<ChefSkillsTypeVO>" />
<!-- 於EL此行可省略 -->
<%-- <jsp:useBean id="chefSkillsSvc" scope="page" class="com.chefskillstype.model.ChefSkillsTypeService" /> --%>
...
<head>
<title>複合查詢 - listChefSkillsType_ByCompositeQuery.jsp</title>
								...
								<main class="main">
									...
									<table>
										...
										<c:forEach var="chefSkillsTypeVO"
											items="${listChefSkillsType_ByCompositeQuery}"
											begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">

											<tr align='center' valign='middle'
												${(chefSkillsTypeVO.skillid==param.skillid) ? 'bgcolor=#CCCCFF':''}>
												<!--將修改的那一筆加入對比色而已-->
												<td>${chefSkillsTypeVO.skillid}</td>
												<td>${chefSkillsTypeVO.skill}</td>

																		<td>
															  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back-end/chefSkillsType/ChefSkillsType.do" style="margin-bottom: 0px;">
															     <input type="submit" value="修改"> 
															     <input type="hidden" name="skillid"      value="${chefSkillsTypeVO.skillid}">
									...
									<%@ include file="../chefSkillsType/page2_ByCompositeQuery.file"%>
								</main>
							</div>
						</div>
						<!-- end container -->
