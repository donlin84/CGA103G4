<%@page import="com.product.controller.GetPmDiscount"%>
...
<Label>&emsp;商品編號:
  <input type="text" id="pdid" name = "Pdid" 
  		value="<%=productVO.getPdid()%>" style="background-color:lightgray;"  readonly/> 
</Label>
</div>
...
<Label>&emsp;優惠價格:
  ...
  <input type="text" id="pdDiscountPrice" name = "PdDiscountPrice" style="background-color:lightgray"
  		value = "<%= Math.round(productVO.getPdPrice()*promotionsVO.getPmDiscount())%>" readonly/>
</Label>
</div>
<%-- ${promotionsVO.pmDiscount * productVO.pdPrice} --%>
<div>
<Label>&emsp;折扣幅度:
  <input type="text" id="pmDiscount" name = "PmDiscount" style="background-color:lightgray" 
...
<Label>&emsp;優惠方案:
       	...
       	<select size="1" name="Pmid" id="Promotion">
				<c:forEach var="promotionsVO" items="${pmtSvc.all}">
				<option value="${promotionsVO.pmid}">${promotionsVO.pmName} </option>
				</c:forEach>
		</select>
...
function KeyP(v){
	document.getElementById("pdDiscountPrice").value = Math.round(v.value * (document.getElementById("pmDiscount").value));
}
