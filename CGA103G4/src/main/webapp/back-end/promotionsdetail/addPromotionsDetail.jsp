										...
										<tr>
											...
											<td><select size="1" name="pdid" id="Product">
													<c:forEach var="ProductVO" items="${pdSvc.all}">
														<option value="${ProductVO.pdid}">${ProductVO.pdName}</option>
													</c:forEach>
											</select></td>
										</tr>
										<tr>
...
window.addEventListener("load", function () {
 ...
 const pmIdVal = document.getElementById('pmIdVal');
 const pdIdVal = document.getElementById('pdIdVal');
 const pmid = document.getElementById('pmid');
 const pdid = document.getElementById('pdid');
 
 Promotion.addEventListener('change',changeVal)
 Product.addEventListener('change',changeVal)
 
 function changeVal(){
	    for(let e of pdid.querySelectorAll('option')){
	                    if(e.selected){
	                        pdIdVal.value = e.innerText;
	                    }
	                }
	                for(let e of pmid.querySelectorAll('option')){
	                    if(e.selected){
	                        pmIdVal.value = e.innerText;
	                    }
	                }
	 DiscountPrice.value = parseInt(pmIdVal.value * pdIdVal.value);
