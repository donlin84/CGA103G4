...
<%
PromotionsVO promotionsVO = (PromotionsVO) request.getAttribute("promotionsVO"); //PromotionsServlet.java (Concroller) 存入req的promotionsVO物件 (包括幫忙取出的empVO, 也包括輸入資料錯誤時的empVO物件)
%>
...
<body bgcolor='white'>
	...
	<FORM METHOD="post" ACTION="PromotionsServlet" name="form1">
			...
			<tr>
				...
				<td><select size="1" name="pmid">
						<c:forEach var="promotionsVO" items="${pmtSvc.all}">
							<option value="${promotionsVO.pmid}"
								${(promotionsVO.pmid==promotionsVO.pmid)?'selected':'' }>${promotionsVO.pmName}
						</c:forEach>
				</select></td>
			</tr>
		...
		<br> <input type="hidden" name="action" value="update"> <input
			type="hidden" name="pmid" value="<%=promotionsVO.getPmid()%>">
