...
<body>
  ...
  <jsp:useBean id="couponTypeVO" class="com.coupontype.model.CouponTypeVO" />
	...
	<div class="Cart-Container">
		...
		<c:forEach var="cartDetailVO" items="${list}">
			<div class="Cart-Items">
				<div class="image-box">
	                <a href="<%=request.getContextPath() %>/front-end/shop/FrontEndPdDetail.do?pdid=${cartDetailVO.productVO.pdid}">
                 <img src="<%=request.getContextPath() %>/front-end/shop/Productpic.do?pdid=${cartDetailVO.productVO.pdid}" style="height:120px">
              </a>
	            </div>
				<div class="title">${cartDetailVO.productVO.pdName}</div>
				<div id="${cartDetailVO.productVO.pdid}_price" class="subtitle" value="${cartDetailVO.productVO.pdPrice}">${cartDetailVO.productVO.pdPrice}</div>
				<div id="${cartDetailVO.productVO.pdid}_discountprice" class="subtitle" value="${cartDetailVO.productVO.pdDiscountPrice}">${cartDetailVO.productVO.pdDiscountPrice}</div>
				<div class="counter">
					<FORM METHOD="post" ACTION="CartDetail.do" style="margin-bottom: 0px;">
						<input class="btn_minus" type="submit" value="-">
						<input type="hidden" name="memid"  value="${cartDetailVO.memid}">
					    <input type="hidden" name="pdid"  value="${cartDetailVO.pdid}">
					    <input type="hidden" name="pdNumber"  value="${cartDetailVO.pdNumber}">
					    <input type="hidden" name="action" value="minus">
					</FORM>
					<div id="${cartDetailVO.productVO.pdid}_num" value="${cartDetailVO.pdNumber}">${cartDetailVO.pdNumber}</div>
					<FORM id="plus" METHOD="post" ACTION="CartDetail.do" style="margin-bottom: 0px;">
						<input class="btn_plus" type="submit" value="+">
						<input type="hidden" name="memid"  value="${cartDetailVO.memid}">
					    <input type="hidden" name="pdid"  value="${cartDetailVO.pdid}">
					...
					<c:otherwise>
						...
						<select name="memCpid" id="memCpid">
							...
							<c:forEach var="memberCouponVO" items="${memCplist}">
								...
								<c:if test="${memberCouponVO.memid == memid}" var="condition" scope="page">
									<c:if test="${memberCouponVO.memCpStatus == 1}" var="condition" scope="page">
										<c:if test="${memberCouponVO.memCpDate >= nowTime}" var="condition" scope="page">
											<option id="${memberCouponVO.memCpid}" value="${memberCouponVO.memCpid}">${memberCouponVO.cpTpVO.cpName}</option>
<%-- 											<option value="">${memberCouponVO.couponTypeVO.cpDiscount}</option> --%>
        ...
        <div class="checkout">
            ...
            <FORM METHOD="post" ACTION="CartDetail.do">
	            <input type="text" class="total-amount" readonly name="total" />
	            <input type="hidden" id="subtotal" name="subtotal">
	            <input type="hidden" name="pdid" value="${pdid}">
	            <input type="hidden" name="pdNumber" value="${pdNumber}">
	            <input type="hidden" name="pdPrice" value="${pdPrice}">
	            <input type="hidden" name="pdDiscount" value="${pdDiscount}">
