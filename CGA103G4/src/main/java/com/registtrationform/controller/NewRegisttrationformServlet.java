...
//			RegisttrationFormVO egf=regSrv.getOneRegisttrationForm(100, 300);
//			req.setAttribute("123", egf);
			//統計 待加強的評價
			List<RegisttrationFormVO> list1=regSrv.review_sum(1, claid);
			Integer sum_1=0; 
			if(list1!=null) {
				for(RegisttrationFormVO R : list1) {
					sum_1+=R.getRegPeople();
				}
				req.setAttribute("review1",sum_1);	
			}else if(list1.isEmpty()){
				req.setAttribute("review1",sum_1);
			}
			
			//統計 尚可的評價
			List<RegisttrationFormVO> list2=regSrv.review_sum(2, claid);
			Integer sum_2=0; 
			if(list2!=null) {
				for(RegisttrationFormVO R : list2) {
					sum_2+=R.getRegPeople();
				}
				req.setAttribute("review2",sum_2);
			...
			//統計 普通的評價
			List<RegisttrationFormVO> list3=regSrv.review_sum(3, claid);
			Integer sum_3=0; 
			if(list3!=null) {
				for(RegisttrationFormVO R : list3) {
					sum_3+=R.getRegPeople();
				}
				req.setAttribute("review3",sum_3);
			...
			//統計 不錯的評價
			List<RegisttrationFormVO> list4=regSrv.review_sum(4, claid);
			Integer sum_4=0; 
			if(list4!=null) {
				for(RegisttrationFormVO R : list4) {
					sum_4+=R.getRegPeople();
				}
				req.setAttribute("review4",sum_4);	
			}else if(list4.isEmpty()){
				req.setAttribute("review4",sum_4);	
			}
			
			//統計 滿意的評價
			List<RegisttrationFormVO> list5=regSrv.review_sum(5, claid);
			Integer sum_5=0; 
			if(list5!=null) {
				for(RegisttrationFormVO R : list5) {
					sum_5+=R.getRegPeople();
				}
				req.setAttribute("review5",sum_5);
