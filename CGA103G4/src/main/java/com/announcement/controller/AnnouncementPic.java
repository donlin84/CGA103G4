...
@WebServlet("/AnnouncementPic")
public class AnnouncementPic extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			String id = req.getParameter("annid").trim();
			ResultSet rs = stmt.executeQuery("SELECT annPic FROM announcement WHERE annid =" + id);
//				 + req.getParameter("PID")

			if (rs.next()) {
				BufferedInputStream in = new BufferedInputStream(rs.getBinaryStream("annPic"));
